...
{
    // アラートに表示するタイトルとメッセージを取得する
    NSDictionary *userInfo = notification.userInfo;
    
    
    // ローカル通知から起動時、アプリ起動中にローカル通知受信時に呼ばれる
    if(notification && userInfo) {
        UIAlertView *alert = [[UIAlertView alloc]
                              initWithTitle:userInfo[@"title"]
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "KentarOu.CoreDataSample" in the application's documents directory.
